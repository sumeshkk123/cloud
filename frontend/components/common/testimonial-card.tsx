...
const TestimonialCard = ({
  ...
  return (
    <article
      className={cn(
        "relative h-full shrink-0 overflow-hidden rounded-[32px] border border-border/40 bg-card/95 p-6 transition hover:border-primary/60 hover:shadow-[0_45px_110px_-65px_rgba(37,99,235,0.35)] w-[calc(100%-3rem)] md:w-[calc(50%-0.75rem)] xl:w-[calc(33.333%-1rem)]",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          ...
          {platform ? (
            href ? (
              <Link href={href} className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80">
                {platform}
              </Link>
            ) : (
              <span className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground">{platform}</span>
            )
          ) : null}
        </div>
      </div>
