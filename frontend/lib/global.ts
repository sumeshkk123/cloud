...
import { fetchStrapi, type StrapiSingleResponse } from "./strapi-client";
...
export async function getGlobalSettings(locale: SupportedLocale): Promise<GlobalSettings | null> {
 const response = await fetchStrapi<StrapiSingleResponse<GlobalAttributes>>("/api/global", {
    params: {
      populate: "*",
      locale
    }
  ...
  const attributes = ((entity as any)?.attributes ?? entity) as GlobalAttributes | null;

  if (!attributes) {
    return null;
  }

  return normalizeGlobal(attributes, locale);
}

function normalizeGlobal(attributes: GlobalAttributes | null | undefined, requestedLocale: SupportedLocale): GlobalSettings {
