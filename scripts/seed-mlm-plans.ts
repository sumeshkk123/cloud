...
async function seedPlans() {
  console.log('Starting to seed MLM plans...');
  ...
  try {
    for (const planData of plans) {
      ...
      if (existing) {
        ...
        console.log(`Updating existing plan: ${planData.title}`);
        ...
        console.log(`✓ Updated: ${planData.title}`);
      } else {
        // Create new plan
        const id = randomUUID();
        ...
        console.log(`✓ Created: ${planData.title}`);
      }
    }

    console.log('\n✅ Successfully seeded all MLM plans!');
  } catch (error) {
