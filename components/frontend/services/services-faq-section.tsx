'use client';

import { useState, useEffect } from "react";
import type { Locale } from "@/i18n-config";
import { SectionTitle } from "@/components/ui/section-title";
import { FaqCard } from "@/components/frontend/common/faq-card";
import { Section } from "@/components/ui/section";

// Translation strings
const translations: Record<Locale, {
  loadingFaqs: string;
  noFaqsAvailable: string;
}> = {
  en: {
    loadingFaqs: "Loading FAQs...",
    noFaqsAvailable: "No FAQs available yet."
  },
  es: {
    loadingFaqs: "Cargando preguntas frecuentes...",
    noFaqsAvailable: "Aún no hay preguntas frecuentes disponibles."
  },
  fr: {
    loadingFaqs: "Chargement des FAQ...",
    noFaqsAvailable: "Aucune FAQ disponible pour le moment."
  },
  it: {
    loadingFaqs: "Caricamento FAQ...",
    noFaqsAvailable: "Nessuna FAQ disponibile ancora."
  },
  de: {
    loadingFaqs: "FAQs werden geladen...",
    noFaqsAvailable: "Noch keine FAQs verfügbar."
  },
  pt: {
    loadingFaqs: "Carregando perguntas frequentes...",
    noFaqsAvailable: "Ainda não há perguntas frequentes disponíveis."
  },
  zh: {
    loadingFaqs: "加载常见问题...",
    noFaqsAvailable: "暂无常见问题。"
  }
};

interface FaqItem {
  id: string;
  question: string;
  answer: string;
}

interface ServicesFaqSectionProps {
  locale: Locale;
}

export function ServicesFaqSection({ locale }: ServicesFaqSectionProps) {
  const [faqs, setFaqs] = useState<FaqItem[]>([]);
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(0);
  const [isLoading, setIsLoading] = useState(false);
  const t = translations[locale];

  useEffect(() => {
    const fetchFaqs = async () => {
      try {
        setIsLoading(true);
        const timestamp = Date.now();

        const res = await fetch(`/api/services-faq?locale=${encodeURIComponent(locale)}&_t=${timestamp}`, {
          cache: 'no-store',
          headers: { 'Cache-Control': 'no-cache' },
        });

        if (res.ok) {
          const data = await res.json();
          const fetchedFaqs = (data?.items || []).map((item: any) => ({
            id: item.id || '',
            question: item.question || '',
            answer: item.answer || '',
          }));
          setFaqs(fetchedFaqs);
          // Open first FAQ by default if available
          if (fetchedFaqs.length > 0) {
            setOpenFaqIndex(0);
          }
        }
      } catch (error) {
        console.error('Error loading service FAQs:', error);
      } finally {
        setIsLoading(false);
      }
    };

    fetchFaqs();
  }, [locale]);

  const handleFaqToggle = (index: number) => {
    setOpenFaqIndex(openFaqIndex === index ? null : index);
  };

  return (
    <Section variant="primary" padding="md" containerClassName="space-y-12">
      <SectionTitle
        badge="FAQ"
        heading="Frequently asked questions"
        description="Answers to the most common questions from leaders weighing Cloud MLM Software as their services partner."
        maxWidth="3xl"
      />
      <div className="space-y-6 max-w-5xl mx-auto">
        {isLoading ? (
          <div className="text-center py-8 text-muted-foreground">
            {t.loadingFaqs}
          </div>
        ) : (
          <>
            <div className="rounded-2xl border border-border/50 bg-card/60 p-8 shadow-sm">
              <div className="space-y-3">
                {faqs.map((faq, faqIndex) => {
                  const isOpen = openFaqIndex === faqIndex;
                  return (
                    <FaqCard
                      key={faq.id}
                      question={faq.question}
                      answer={faq.answer}
                      isOpen={isOpen}
                      onToggle={() => handleFaqToggle(faqIndex)}
                    />
                  );
                })}
                {faqs.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    {t.noFaqsAvailable}
                  </div>
                )}
              </div>
            </div>
          </>
        )}
      </div>
    </Section>
  );
}
