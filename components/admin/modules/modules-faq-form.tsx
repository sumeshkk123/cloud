...
interface ModuleFaqTranslation {
    locale: string;
    question: string;
    answer: string;
    exists: boolean;
...
export function ModulesFaqForm({
    faqId,
    onClose,
    onSave,
    onToastChange,
    ...
    const [translations, setTranslations] = useState<Record<string, ModuleFaqTranslation>>(() => {
        const initial: Record<string, ModuleFaqTranslation> = {};
        locales.forEach((loc) => {
            initial[loc] = {
                locale: loc,
                question: '',
    ...
    useEffect(() => {
        if (currentFaqId) {
            ...
            const reset: Record<string, ModuleFaqTranslation> = {};
            locales.forEach((loc) => {
                reset[loc] = {
                    locale: loc,
                    question: '',
    ...
    const loadAllTranslations = async (preserveActiveTab: boolean = false) => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/modules-faq?id=${encodeURIComponent(currentFaqId)}&all=true`);

            if (!response.ok) {
                showToast('Failed to load module FAQ translations.', 'error');
                return;
            }
            ...
            const loaded: Record<string, ModuleFaqTranslation> = {};
    ...
    const handleSaveCurrentTab = async () => {
        ...
        try {
            ...
            const idToUse = currentFaqId; // Use currentFaqId directly
            ...
            if (!idToUse) {
                ...
                const res = await fetch('/api/admin/modules-faq', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (!res.ok) throw new Error(payload?.error || 'Failed to save module FAQ');
                ...
                showToast('Module FAQ created successfully.', 'success');
                setIsSaving(false);
                if (onSave) {
                    await onSave();
                }
            ...
            const res = await fetch(`/api/admin/modules-faq?id=${encodeURIComponent(idToUse)}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    question: trimmedQuestion,
            ...
            // Update state with saved values - ensure they persist
            setTranslations((prev) => ({
                ...prev,
                [activeTab]: {
                    ...prev[activeTab],
            ...
            const message = error instanceof Error ? error.message : 'Unable to save module FAQ.';
