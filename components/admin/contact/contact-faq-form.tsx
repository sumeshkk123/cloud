...
export function ContactFaqForm({
    ...
    const loadAllTranslations = async (preserveActiveTab: boolean = false) => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/contact-faq?id=${encodeURIComponent(currentFaqId)}&all=true`);

            if (!response.ok) {
                showToast('Failed to load contact FAQ translations.', 'error');
                return;
            }
    ...
    const handleSaveCurrentTab = async () => {
        ...
        try {
            ...
            if (!idToUse) {
                ...
                const res = await fetch('/api/admin/contact-faq', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (!res.ok) throw new Error(payload?.error || 'Failed to save contact FAQ');
                ...
                showToast('Contact FAQ created successfully.', 'success');
                setIsSaving(false);
                if (onSave) {
                    await onSave();
                }
            ...
            const res = await fetch(`/api/admin/contact-faq?id=${encodeURIComponent(idToUse)}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    question: trimmedQuestion,
            ...
            const message = error instanceof Error ? error.message : 'Unable to save contact FAQ.';
