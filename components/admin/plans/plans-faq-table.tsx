...
export function PlansFaqTable() {
    const { showToast, ToastComponent } = useToast();
    const [faqs, setFaqs] = useState<PlanFaqRow[]>([]);
    ...
    const loadFaqs = async () => {
        try {
            ...
            const res = await fetch(`/api/admin/plans-faq?locale=en&withTranslations=true&_t=${timestamp}`, {
                cache: 'no-store',
            ...
            if (!res.ok) {
                if (res.status >= 500) {
                    showToast(data?.error || 'Unable to load plan FAQs.', 'error');
                }
                setFaqs([]);
                setCurrentPage(1);
                setIsLoading(false);
            ...
            const items: PlanFaqRow[] = Array.isArray(data)
                ? data.map((item: any) => ({
                    id: String(item.id),
            ...
            showToast('Failed to load plan FAQs. Please try again.', 'error');
            setFaqs([]);
            setCurrentPage(1);
        } finally {
            setIsLoading(false);
    ...
    const columns = [
        ...
        { key: 'languages', label: 'Language', className: 'w-32' },
    ...
    const handleDelete = async () => {
        ...
        try {
            setIsDeleting(true);
            const res = await fetch(`/api/admin/plans-faq?id=${encodeURIComponent(faqToDelete)}`, {
                method: 'DELETE',
            });

            if (!res.ok) {
                const data = await res.json();
                showToast(data?.error || 'Failed to delete plan FAQ.', 'error');
                return;
            }

            showToast('Plan FAQ deleted successfully.', 'success');
            setDeleteConfirmOpen(false);
            setFaqToDelete(null);
            await loadFaqs();
        } catch (error) {
            showToast('Failed to delete plan FAQ.', 'error');
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    return (
        <>
                ...
                {isLoading ? (
                    ...
                    <>
                        <Table
                            columns={columns}
                            data={paginated}
                            emptyMessage={isLoading ? 'Loading plan FAQs...' : 'No plan FAQs available yet.'}
                            renderCell={(column, row) => {
                                ...
                                if (column.key === 'answer') {
                                    return <span className="text-gray-600 dark:text-gray-400">{row.answer}</span>;
                                }
                                ...
                                return row[column.key as keyof PlanFaqRow] as string;
                            }}
                        />
            ...
            <Modal
                ...
                title={editingFaqId ? 'Edit Plan FAQ' : 'New Plan FAQ'}
                size="3xl"
            ...
            <DeleteConfirmModal
                ...
                title="Delete Plan FAQ"
                message="Are you sure you want to delete this plan FAQ? This action cannot be undone."
            />
