...
export function PlansFaqForm({
    ...
    const [translations, setTranslations] = useState<Record<string, PlanFaqTranslation>>(() => {
        const initial: Record<string, PlanFaqTranslation> = {};
        locales.forEach((loc) => {
            initial[loc] = {
                locale: loc,
                question: '',
    ...
    useEffect(() => {
        if (currentFaqId) {
            ...
            const reset: Record<string, PlanFaqTranslation> = {};
            locales.forEach((loc) => {
                reset[loc] = {
                    locale: loc,
                    question: '',
    ...
    const loadAllTranslations = async () => {
        ...
        try {
            setIsLoading(true);
            const res = await fetch(`/api/admin/plans-faq?id=${encodeURIComponent(currentFaqId)}&all=true`);
            if (!res.ok) {
                showToast('Failed to load plan FAQ translations.', 'error');
                setIsLoading(false);
                return;
            }
            ...
            const loaded: Record<string, PlanFaqTranslation> = {};
            const existingLocales: string[] = [];

            locales.forEach((loc) => {
                const existing = existingTranslations.find((t: { locale: string }) => t.locale === loc);
                if (existing) {
                    loaded[loc] = {
                        locale: loc,
    ...
    const handleSaveCurrentTab = async () => {
        ...
        try {
            ...
            const url = idToUse === 'new'
                ? '/api/admin/plans-faq'
                : `/api/admin/plans-faq?id=${encodeURIComponent(idToUse)}`;
            ...
            if (!res.ok) throw new Error(payload?.error || 'Failed to save plan FAQ');
            ...
            showToast('Plan FAQ saved successfully.', 'success');
            setIsSaving(false);
            if (onSave) {
                await onSave();
            }
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unable to save plan FAQ.';
            showToast(message, 'error');
            setIsSaving(false);
        }
    };

    const current = translations[activeTab] || { question: '', answer: '', locale: activeTab, exists: false };
            ...
            {activeTab !== 'en' && translations['en']?.question && translations['en']?.answer && (
                <div className="flex justify-end">
                    <button
                        ...
                        disabled={isTranslating || !translations['en']?.question.trim() || !translations['en']?.answer.trim()}
                    >
                        {isTranslating ? (
                            <>
