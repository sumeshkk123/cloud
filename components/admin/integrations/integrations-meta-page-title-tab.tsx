...
import { IntegrationsMetaPageTitleForm } from './integrations-meta-page-title-form';
...
const INTEGRATION_PAGE_PREFIX = 'integrations';
const ITEMS_PER_PAGE = 20;

export function IntegrationsMetaPageTitleTab() {
  ...
  const [integrationPages, setIntegrationPages] = useState<Array<{ value: string; label: string }>>([]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- loadAllData runs when integrationPages changes
  ...
  const loadAllData = async () => {
    if (integrationPages.length === 0) return;
    
    try {
      setIsLoading(true);
      const allData: CombinedRow[] = [];
      
      // Load data for all integration pages (excluding main integrations page)
      for (const pageOption of integrationPages) {
        const page = pageOption.value;
        
        // Skip the main integrations page - only show inner pages
        if (page === INTEGRATION_PAGE_PREFIX) {
          continue;
        }
  ...
  const loadIntegrationPages = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const dbPages = integrations.map((i: any) => ({
          value: `${INTEGRATION_PAGE_PREFIX}/${i.id}`,
          label: i.title || i.id,
        }));
      ...
      // Don't add main integrations page - only show inner pages
      // Sort alphabetically by label
      allPages.sort((a, b) => a.label.localeCompare(b.label));
      setIntegrationPages(allPages);
    } catch (error) {
      console.error('Error loading integration pages:', error);
      showToast('Failed to load integration pages.', 'error');
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Table
        ...
        renderCell={(column, row) => {
          if (column.key === 'page') {
            ...
            const pageSlug = row.page.replace(`${INTEGRATION_PAGE_PREFIX}/`, '');
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end items-center gap-2">
            ...
            <Button
              type="submit"
              form="integrations-meta-page-title-form"
              variant="primary"
      ...
      >
        <IntegrationsMetaPageTitleForm
          initialPage={editingPage || undefined}
          initialLocale={editingLocale}
          integrationPages={integrationPages}
          onClose={() => {
            setIsFormOpen(false);
            setEditingPage(null);
            setEditingLocale('en');
