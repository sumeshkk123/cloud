...
interface ServicesFaqFormProps {
    faqId?: string | null;
    onClose: () => void;
    onSave?: () => void;
...
export function ServicesFaqForm({
    faqId,
    onClose,
    onSave,
    onToastChange,
    ...
    const [translations, setTranslations] = useState<Record<string, ServiceFaqTranslation>>(() => {
        const initial: Record<string, ServiceFaqTranslation> = {};
        locales.forEach((loc) => {
            initial[loc] = {
                locale: loc,
                question: '',
    ...
    useEffect(() => {
        if (currentFaqId) {
            ...
            const reset: Record<string, ServiceFaqTranslation> = {};
            locales.forEach((loc) => {
                reset[loc] = {
                    locale: loc,
                    question: '',
    ...
    const loadAllTranslations = async (preserveActiveTab: boolean = false) => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/services-faq?id=${encodeURIComponent(currentFaqId)}&all=true`);

            if (!response.ok) {
                showToast('Failed to load service FAQ translations.', 'error');
                return;
            }
            ...
            const loaded: Record<string, ServiceFaqTranslation> = {};
    ...
    const handleSaveCurrentTab = async () => {
        ...
        try {
            ...
            if (!idToUse) {
                ...
                const res = await fetch('/api/admin/services-faq', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (!res.ok) throw new Error(payload?.error || 'Failed to save service FAQ');
                ...
                showToast('Service FAQ created successfully.', 'success');
                setIsSaving(false);
                if (onSave) {
                    await onSave();
                }
            ...
            const res = await fetch(`/api/admin/services-faq?id=${encodeURIComponent(idToUse)}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    question: trimmedQuestion,
            ...
            const message = error instanceof Error ? error.message : 'Unable to save service FAQ.';
