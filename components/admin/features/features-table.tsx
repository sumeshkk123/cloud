...
export function FeaturesTable() {
  ...
  const loadItems = async (showLoading = true) => {
    try {
      ...
      if (!res.ok) {
        if (res.status >= 500) {
          showToast(data?.error || 'Unable to load features.', 'error');
        }
        setItems([]);
        setCurrentPage(1);
        return;
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const res = await fetch(`/api/admin/features?id=${encodeURIComponent(deleteId)}`, {
        method: 'DELETE',
      });

      if (!res.ok) {
        const data = await res.json();
        showToast(data?.error || 'Failed to delete feature.', 'error');
        return;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <Table
        ...
        data={paginated}
        isLoading={isLoading}
        emptyMessage={isLoading ? 'Loading features...' : 'No features available yet.'}
        renderCell={(column, row) => {
          ...
          if (column.key === 'category') {
            const translatedCategory = getTranslatedCategory(row.category, 'en');
            return <span className="text-sm text-gray-600">{translatedCategory}</span>;
          }
          if (column.key === 'slug') {
          ...
          return row[column.key as keyof FeatureRow] as string;
        }}
      />
      ...
      >
        <FeaturesForm
          featureId={editingId}
          onClose={() => {
            // Don't close modal on save, just reload items
          }}
          onSave={async () => {
          ...
          onSaveRequest={() => {
            // This will be called when save button in footer is clicked
          }}
        />
        {formToast}
      </Modal>
