...
export function FeaturesFaqTable() {
    const { showToast, ToastComponent } = useToast();
    const [faqs, setFaqs] = useState<FeatureFaqRow[]>([]);
    ...
    const loadFaqs = async (showLoading = true) => {
        try {
            ...
            if (!res.ok) {
                if (res.status >= 500) {
                    showToast(data?.error || 'Unable to load feature FAQs.', 'error');
                }
                setFaqs([]);
                setCurrentPage(1);
                setIsLoading(false);
            ...
            const items: FeatureFaqRow[] = Array.isArray(data)
                ? data.map((item: any) => ({
                    id: String(item.id),
                ...
                items.map(async (item) => {
                    try {
                        const translationsRes = await fetch(
                            `/api/admin/features-faq?id=${encodeURIComponent(item.id)}&all=true`,
                            { cache: 'no-store' }
                        );
    ...
    const handleDelete = async () => {
        ...
        try {
            setIsDeleting(true);
            const res = await fetch(`/api/admin/features-faq?id=${encodeURIComponent(faqToDelete)}`, {
                method: 'DELETE',
            });
        ...
        <div className="space-y-4">
            ...
            <Table
                ...
                renderCell={(column, row) => {
                    ...
                    return row[column.key as keyof FeatureFaqRow] as string;
                }}
            />
