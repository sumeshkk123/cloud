...
import {
  ...
  updateModuleFaq,
  deleteModuleFaq,
  getAllModuleFaqTranslations,
} from '@/lib/api/modules-faq';
...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      if (allTranslations) {
        const translations = await getAllModuleFaqTranslations(id);
        return NextResponse.json({ translations });
      }

      const faq = await getModuleFaqById(id, locale);
      if (!faq) {
        return NextResponse.json({ error: 'Module FAQ not found.' }, { status: 404 });
      }
      return NextResponse.json(faq);
    }

    const faqs = await listModuleFaqs('en');
...
export async function POST(request: Request) {
  try {
    ...
    const faq = await createModuleFaq({
...
export async function PUT(request: Request) {
  try {
    ...
    const existing = await getModuleFaqById(id);
    if (!existing) {
      // If FAQ doesn't exist, check if we're creating a translation
      if (locale !== 'en') {
        const englishFaqs = await listModuleFaqs('en');
    ...
    if (targetLocale !== existing.locale) {
      const allTranslations = await getAllModuleFaqTranslations(existing.id);
      ...
      if (existingTranslation) {
        ...
        const faq = await updateModuleFaq(existing.id, {
          question: String(question),
          answer: String(answer),
          locale: targetLocale,
        });
    ...
    const faq = await updateModuleFaq(id, {
      question: String(question),
      answer: String(answer),
      locale: targetLocale,
    });
