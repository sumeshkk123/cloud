...
import {
  ...
  updateServiceFaq,
  deleteServiceFaq,
  getAllServiceFaqTranslations,
} from '@/lib/api/services-faq';
...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      if (allTranslations) {
        const translations = await getAllServiceFaqTranslations(id);
        return NextResponse.json({ translations });
      }

      const faq = await getServiceFaqById(id, locale);
      if (!faq) {
        return NextResponse.json({ error: 'Service FAQ not found.' }, { status: 404 });
      }
      return NextResponse.json(faq);
    }

    const faqs = await listServiceFaqs('en');
...
export async function PUT(request: Request) {
  try {
    ...
    const existing = await getServiceFaqById(id);
    if (!existing) {
      // If FAQ doesn't exist, check if we're creating a translation
      if (locale !== 'en') {
        const englishFaqs = await listServiceFaqs('en');
    ...
    if (targetLocale !== existing.locale) {
      const allTranslations = await getAllServiceFaqTranslations(existing.id);
      ...
      if (existingTranslation) {
        ...
        const faq = await updateServiceFaq(existing.id, {
          question: String(question),
          answer: String(answer),
          locale: targetLocale,
        });
    ...
    const faq = await updateServiceFaq(id, {
      question: String(question),
      answer: String(answer),
      locale: targetLocale,
    });
