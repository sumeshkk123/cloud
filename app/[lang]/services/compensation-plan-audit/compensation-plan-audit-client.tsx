...
import { compensationPlanAuditContent } from "./content";

type CompensationPlanAuditClientProps = {
...
export function CompensationPlanAuditClient({
  pageTitleData,
  contactHref,
  secondaryHref,
  locale = "en",
}: CompensationPlanAuditClientProps) {
  const content = mergeContentWithPageTitle(
    compensationPlanAuditContent,
    pageTitleData
  );
  return (
    <ServiceSubpageLayout
      ...
      serviceSlug="compensation-plan-audit"
      locale={locale}
    />
  );
}
