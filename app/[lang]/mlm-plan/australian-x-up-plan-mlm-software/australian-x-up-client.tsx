...
import { australianXUpPlanContent } from "./content";
import { AustralianXUpSections } from "./sections";
...
export function AustralianXUpClient({
  pageTitleData,
  contactHref,
  demoHref,
  plansHref,
  locale = "en",
}: AustralianXUpClientProps) {
  const [simulatorModalOpen, setSimulatorModalOpen] = useState(false);
  const content = mergeContentWithPageTitle(australianXUpPlanContent, pageTitleData);
  ...
  return (
    <>
      ...
      >
        <AustralianXUpSections plansHref={plansHref} />
      </PlanSubpageLayout>
      ...
      {simulatorModalOpen && (
        ...
        >
          ...
          >
            ...
            <div className="p-6">
              <PassUpFlowSimulator className={SIMULATOR_MODAL_CLASS} />
            </div>
          </div>
        </div>
      )}
