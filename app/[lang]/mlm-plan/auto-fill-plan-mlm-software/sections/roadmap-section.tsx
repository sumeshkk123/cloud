...
function StepContent({
  ...
  return (
    <div className={cn("min-w-0 pt-0.5", isRight ? "text-left" : "text-right")}>
      ...
      <ul className={cn("mt-3 space-y-1.5", isRight ? "text-left" : "text-right")}>
        {step.activities.map((activity, i) => (
          <li
            key={i}
            className={cn("flex gap-2 text-sm text-muted-foreground", isRight ? "flex-row" : "flex-row-reverse")}
          >
          ...
          {roadmap.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn("col-start-2 min-w-0 pl-4 md:col-start-3", isLeft && "md:hidden")}>
